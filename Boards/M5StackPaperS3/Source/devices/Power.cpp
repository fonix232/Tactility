...
std::shared_ptr<PowerDevice> createPower() {
    // PaperS3Power::Configuration configuration;
    // configuration.adcMultiplier = 2.0f; // Voltage divider ratio

    return std::make_shared<PaperS3Power>();
}

bool PaperS3Power::adcInitCalibration() {
...
uint32_t PaperS3Power::adcReadValue() const {
...
bool PaperS3Power::ensureInitialized() {
    if (!initialized) {
...
bool PaperS3Power::getMetric(MetricType type, MetricData& data) {
    ...
    switch (type) {
        ...
        case MetricType::IsCharging:
            data.valueAsBool = false; // TODO: implement charging detection
            return true;
        default:
            return false;
    }
